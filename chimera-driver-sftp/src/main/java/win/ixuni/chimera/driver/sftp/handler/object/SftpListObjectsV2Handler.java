 ...
 * SFTP 列出对象处理器 (V2)
 */
public class SftpListObjectsV2Handler implements OperationHandler<ListObjectsV2Operation, ListObjectsV2Result> {
    ...
    public Mono<ListObjectsV2Result> handle(ListObjectsV2Operation operation, DriverContext context) {
        ...
        // continuationToken is the last key from the previous page
        String marker = continuationToken != null ? continuationToken : startAfter;
        ...
        return Mono.fromCallable(() -> {
            try (SftpClient sftpClient = ctx.createSftpClient()) {
                ...
                String nextContinuationToken = isTruncated ? objects.get(objects.size() - 1).getKey() : null;
